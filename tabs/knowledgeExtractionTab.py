...
with gr.Blocks() as knowledgeExtractionTab:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            dropdownKEModel = gr.Dropdown(label='Model for Knowledge Extraction',value='spacy', choices=['spacy','rebel'])
    ...
    # Functions
    getTweetButton.click(fn=twitterFunctions.getTweet, inputs=tweetQueryText, outputs=keInputText)  
    deleteTextButton.click(fn=lambda:"", inputs=[], outputs=keInputText)
    runKEButton.click(fn=callPipeline, inputs=[keInputText, dropdownKEModel], outputs=kgInJSONLD)
