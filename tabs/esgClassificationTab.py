...
def esg(question):
    try:
        ...
        request = requests.post(url, json={"messages": [{"role": "user",
                                                "content": f"Which class does this text best fall into? Environment, Social, Governance or out-of-scope? Note that out-of-scope refers to any class other than Environment, Social, Governance. Answer with a single word.\n\n{question}"}],
                                "temperature": 0.1,
                                "max_new_tokens": 10,
